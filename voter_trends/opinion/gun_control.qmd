...
:::{.toggle-div .visible}
```{r guns-income}
p + guides(colors=guide_legend(nrow=2))
```
...
The summary statistics are available [here](data/opinion_Gun Control.csv). The `Group N` column is the number of respondents in a given group in a given year while the `N` column is the number of respondents who selected the answer in the `Gun Control` column.
