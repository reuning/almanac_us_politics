...
The summary statistics are available [here](data/opinion_Civil Rights Speed.csv). The `Group N` column is the number of respondents in a given group in a given year while the `N` column is the number of respondents who selected the answer in the `Civil Rights Speed` column.
